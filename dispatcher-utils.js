const TRANSIENT_ERROR_PATTERNS = [
  ...
  // "retry after" / "retry in" require a number with a word boundary after it:
  // gateway rate-limit/overload notices say "retry after 30 seconds" (or
  // surface a Retry-After value), while agent reports can contain the prose
  // "the retry after this morning's attempt" (observed false positive:
  // Morning Daily Brief 2026-09-08, run 60c3cebf). The trailing \b (on \d+) is
  // preserved from the original combined pattern so e.g. "retry in 2FA" does
  // not match.
  /\bretry\s+after\s+\d+\b/i,
  /\bretry\s+in\s+\d+\b/i,
  /\bretry\s+later\b/i,
  ...
  // "upstream provider overloaded" (overloaded, 502). Observed in prod (rh-bot,
  // 2026-09-09, Daily Brief): "Chat completions stream error: upstream provider
  // timeout" soft-failed with no retry because no existing pattern covered the
  // provider-prefix form. The triple-word anchor keeps prose like "the upstream
  // provider was down" unflagged; verified 0 false-positives across all 1,740
  // historical ok-run summaries in the prod scheduler DB.
  /\bupstream\s+provider\s+(?:timeout|error|overloaded)\b/i,
];
