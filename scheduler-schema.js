...
export const SCHEDULER_SCHEMAS = {
  jobs: {
    ...
    fields: {
      ...
      payload_model_fallback: { type: 'string', nullable: true, description: 'Optional fallback model override for a same-run retry; an explicit or inherited effective auth profile requires a concrete provider/model' },
      ...
      auth_profile: { type: 'string', nullable: true, description: 'Auth profile override: null=default, "inherit"=main session profile, or an explicit profile ID. Isolated agent selections with a profile require a concrete provider/model; routing-only models are incompatible.' },
      auth_profile_fallback: { type: 'string', nullable: true, description: 'Optional fallback auth profile for a same-run retry; uses the fallback model or primary model when absent. The effective pair is validated before persistence.' },
