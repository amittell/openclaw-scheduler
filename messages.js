...
export function sendMessage(opts) {
  ...
  db.prepare(`
    INSERT INTO messages (id, from_agent, to_agent, reply_to, kind, subject, body, metadata, priority, channel, status, expires_at, job_id, run_id)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?)
  `).run(
    id,
    opts.from_agent,
...
/**
 * Get pending messages for an agent (inbox), ordered by priority then time.
 */
